# The Orphan — Cross‑Covenant Law & Structural Overview

[Live site](https://raphaelasanti.github.io/the-orphan/) • [Full ebook (The End of Time)](https://raphaelasanti.github.io/the-end-of-time/) • [License](./LICENSE)

A structural overview of the orphan‑law across Judaism, Christianity, and Islam, demonstrating its continuity, its non‑negotiable status, and its direct divine enforcement. This project hosts a focused HTML page presenting the cross‑covenant boundary that no group can bypass: the orphan‑law contains no loopholes, no exceptions, and no interpretive escape routes.

---
![License: All Rights Reserved](https://img.shields.io/badge/License-All%20Rights%20Reserved-lightgrey.svg)
[![GitHub Pages](https://img.shields.io/badge/site-GitHub%20Pages-green.svg)](https://raphaelasanti.github.io/the-orphan/)

---

## Table of contents
- [About](#about)
- [Files / Links](#files--links)
- [How to view](#how-to-view)
- [License](#license)
- [Author](#author)
- [Donate](#donate)
- [Contribute](#contribute)
- [Related pages](#related-pages)

## About
This repo contains a focused HTML page presenting the orphan‑law as the single cross‑covenant statute that remains unchanged across Judaism, Christianity, and Islam. The page outlines the direct divine jurisdiction over the fatherless, the absence of scriptural loopholes, and the judicial consequences described in the texts. It concludes with the section **“The Inviolable Law Even Cain Cannot Escape,”** demonstrating that Cain’s protection applies only to human hands, not to God Almighty Himself.

## Files / Links
- index.html — Primary page (HTML)  
  https://raphaelasanti.github.io/the-orphan/
- Full Ebook — The End of Time (HTML)  
  https://raphaelasanti.github.io/the-end-of-time/
- Source repo for the full ebook  
  https://github.com/raphaelasanti/the-end-of-time/

## How to view
Open the live site URL above (GitHub Pages). The page is served via GitHub Pages — no installation required.

## License
All rights reserved.  
See the repository’s [LICENSE](./LICENSE) file for details.

## Author
Raphael E. Asanti — 2026

## Donate
This work is sustained entirely by voluntary contributions. Thank you for considering support.

- Cash App: https://cash.app/$phirunltd  
- Gumroad (optional donation when downloading): https://phirun4.gumroad.com/l/zsmzjh

(If you prefer not to donate, the materials are available for free — see “Related pages.”)

## Contribute
- Share the public page on social media to extend its reach:  
  https://raphaelasanti.github.io/the-orphan/
- Report issues or suggest edits via GitHub issues in this repo. Pull requests welcome for documentation fixes or accessibility improvements.

## Related pages
- Main Index: https://raphaelasanti.github.io/
- Full eBook: https://github.com/raphaelasanti/the-end-of-time/
- Elijah's Burned Bull: https://raphaelasanti.github.io/burned-bull-elijah/
- The Real Reason for Earth's Tilt: https://raphaelasanti.github.io/tilt-of-the-earth/
- The Eye of the Needle: https://raphaelasanti.github.io/the-eye-of-the-needle/
- Woman in the Wilderness: https://raphaelasanti.github.io/woman-in-the-wilderness/

